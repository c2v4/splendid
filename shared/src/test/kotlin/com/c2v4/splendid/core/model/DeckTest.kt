...
internal class DeckTest {
    ...
    fun createWithoutFirstTest() {
        ...
        assertEquals(deck!!.peek().points, 1)
        deck = Deck.withoutFirst(deck!!)
        assertEquals(4, deck!!.size())
        assertEquals(deck!!.peek().points, 2)
        deck = Deck.withoutFirst(deck!!)
        assertEquals(3, deck!!.size())
        assertEquals(deck!!.peek().points, 3)
        deck = Deck.withoutFirst(deck!!)
        assertEquals(2, deck!!.size())
        assertEquals(deck!!.peek().points, 4)
        deck = Deck.withoutFirst(deck!!)
        assertEquals(1, deck!!.size())
        assertEquals(deck!!.peek().points, 5)
        deck = Deck.withoutFirst(deck!!)
